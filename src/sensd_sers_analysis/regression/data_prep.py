...
from sensd_sers_analysis.classification.data_prep import prepare_phase2_data
...
def prepare_concentration_regression_data(
    ...
    *,
    ...
    Reuses Phase 2 cleaning (Pass sensors, inlier filtering, serotype / Rinsate
    labeling). Retains only **non-Rinsate** rows with **positive** concentration
    and finite ``log_concentration`` (one row-level serotype label per sample).

    Parameters
    ----------
    df:
        Filtered feature dataframe from Phase 1.
    excluded_map, feature_cols, inlier_feature:
        Passed through to :func:`~sensd_sers_analysis.classification.prepare_phase2_data`.
    ...
    if log_conc_col not in phase2.columns:
        return pd.DataFrame()

    out = phase2[phase2[target_col].astype(str) != "Rinsate"].copy()
